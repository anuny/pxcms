(function(w, undefined) {
	...
	function getNotice(){
		var arr,reg=new RegExp("(^| )__px_notice_msg=([^;]*)(;|$)");
		if(arr=document.cookie.match(reg)){
			var notice = decodeURI(unescape(arr[2])).split('||');
			var myDate=new Date();    
			myDate.setTime(-1000);//设置时间    
			document.cookie="__px_notice_msg=null; expires="+myDate.toGMTString();
			return {status:notice[0],msg:notice[1]};
		}
	}
	
	var notice = getNotice();
	if(notice){
		var type = notice.status ? 'notice-success' : 'notice-error';
		$('body').append('<div class="notice '+ type +'">'+ notice.msg +'</div>');
		setTimeout(function(){
			$('.notice').remove();
		},3000)
	}
